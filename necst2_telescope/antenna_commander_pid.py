...
class antenna_feedback(object):
    ...
    def __init__(self):
        ...
        self.p_coeff = float(self.node.declare_parameter("p_coeff").value)
        self.i_coeff = float(self.node.declare_parameter("i_coeff").value)
        self.d_coeff = float(self.node.declare_parameter("d_coeff").value)

        self.hensa_stock = [0] * self.i_ave_num

        self.gear_ratio = float(self.node.declare_parameter("gear_ratio").value)
        self.pulseper360deg = float(self.node.declare_parameter("pulseper360deg").value)
        self.pulse_a = float(self.node.declare_parameter("pulse_a").value)
        self.pulse_b = float(self.node.declare_parameter("pulse_b").value)

        self.MOTOR_MAXSTEP = float(self.node.declare_parameter("MOTOR_MAXSTEP").value)
        self.MOTOR_MAXSPEED = float(self.node.declare_parameter("MOTOR_MAXSPEED").value)

        map_azel = {"az": "y", "el": "x"}  # noqa: F841
        topic_name = {
            "to": "pyinterface/pci7415/rsw0/azel_xy/speed_cmd",
            "cur": "opu1p85m/azel/current_speed",
            "tar": "opu1p85m/azel/target_speed",
            "hensa": "opu1p85m/azel/pid_hensa",
            "from1": "opu1p85m/azel/cmd2",
            "from2": "dev/HEIDENHAIN/ND287/azel",
        }
        ...
        topic_from1 = self.node.create_subscription(  # noqa: F841
            Float64, topic_name["from1"], self.antenna_feedback, 1
        )
        topic_from2 = self.node.create_subscription(  # noqa: F841
            Float64, topic_name["from2"], self.antenna_encoder, 1
        )
        pass
    ...
    def antenna_feedback(self, command):
        ...
        speed = (
            ...
            * self.gear_ratio
            / 360.0
            * (self.pulseper360deg * (self.pulse_b / self.pulse_a))
        )
        ...
        if abs(speed - self.speed_d) < self.MOTOR_MAXSTEP:
            ...
            a = np.sign(speed - self.speed_d)
            self.speed_d += a * self.MOTOR_MAXSTEP

        # limit max speed
        sign = np.sign(self.speed_d)
        if abs(self.speed_d) > self.MOTOR_MAXSPEED:
            self.speed_d = sign * self.MOTOR_MAXSPEED
    ...
    def calc_pid(self):
        ...
        if (self.encoder_deg - self.enc_before) != 0.0:
            self.current_speed = (self.encoder_deg - self.enc_before) / (
        ...
        rate = (
            target_speed
            + self.p_coeff * hensa
            + self.i_coeff * self.ihensa * (self.t_now - self.t_past)
            + self.d_coeff * dhensa / (self.t_now - self.t_past)
        )
