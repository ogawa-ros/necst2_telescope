...
class pci7415_driver(object):
    ...
    #元handler 命令値を受け取る
        ...
        for ax in self.use_axis:
            ...
            self.node.create_subscription(std_msgs.msg.Int64, b+'step_cmd', self.set_step, callback_args=ax)

            # rospy.Subscriber(b+'speed_cmd', std_msgs.msg.Float64, self.set_speed, callback_args=ax)
            self.node.create_subscription(std_msgs.msg.Int64, b+'speed_cmd', self.set_speed, callback_args=ax)

        for do_num in range(1,5):
            self.node.create_subscription(std_msgs.msg.Int64, '{}/output_do{}_cmd'.format(base, do_num), self.set_do, callback_args=do_num)
...
if __name__ == '__main__':
    ...
    self.use_axis = self.node.get_parameter('~use_axis').get_parameter_value().string_value
